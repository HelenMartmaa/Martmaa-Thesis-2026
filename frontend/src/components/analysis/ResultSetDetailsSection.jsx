...
function ResultSetDetailsSection({
  ...
  const isLinkedExperimentLocked = isEditMode && datasetSource === "linked";
  ...
  const handleLinkedExperimentChange = (event) => {
    if (isLinkedExperimentLocked) {
      return;
    }
    ...
    const selected = experiments.find(
      (experiment) => String(experiment.id) === String(selectedExperimentId)
    );

    setFormData((prev) => ({
      ...prev,
      experimentId: selectedExperimentId,
      experimentType: selected?.experimentType || prev.experimentType,
    }));
  };
        ...
        {loadingExperiments ? (
          ...
          <>
            <div className="space-y-3">
              ...
              {isEditMode && (
                <p className="text-xs text-slate-500">
                  Dataset source cannot be changed after creation. Create a new
                  dataset if you need a different source type.
                </p>
              )}
            </div>
            ...
            {datasetSource === "linked" && (
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={handleLinkedExperimentChange}
                  disabled={isLinkedExperimentLocked}
                  className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500"
                >
                ...
                {isLinkedExperimentLocked && (
                  <p className="text-xs text-slate-500">
                    The linked experiment cannot be changed after the result
              ...
              {datasetSource === "linked" ? (
                <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    This value is inherited from the linked planned experiment
